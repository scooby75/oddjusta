...
def mostrar_resultados_h2h(time_home, time_away):
    ...
    st.dataframe(h2h_df[['Data', 'Home', 'Away', 'Odd_Home', 'Odd_Empate', 'Odd_Away', 'Gols_Home', 'Gols_Away', 'Resultado', 'Coeficiente_Eficiencia', 'Placar']])

    calcular_estatisticas_e_exibir(h2h_df, "Home", 'Odd_Home')

def calcular_estatisticas_e_exibir(df, team_type, odds_column):
    # Calcular estatísticas de vitórias, empates e derrotas
    wins = df[df['Resultado'] == 'W'].shape[0]
    draws = df[df['Resultado'] == 'D'].shape[0]
    losses = df[df['Resultado'] == 'L'].shape[0]
    ...
    st.write(f"Total de jogos: {total_games}")
    st.write(f"Vitórias: {wins} ({win_rate:.2f}%)")
    st.write(f"Empates: {draws} ({draw_rate:.2f}%)")
    st.write(f"Derrotas: {losses} ({loss_rate:.2f}%)")

    # Calcular lucro/prejuízo
    df['Lucro_Prejuizo'] = df.apply(lambda row: row[odds_column] - 1 if row['Resultado'] == 'W' else -1, axis=1)
    total_profit_loss = df['Lucro_Prejuizo'].sum()
    st.write(f"Lucro/Prejuízo Total: {total_profit_loss:.2f}")

    # Calcular placar mais comum
    placar_mais_comum = df['Placar'].mode().values[0] if not df['Placar'].mode().empty else "N/A"
    st.write(f"Placar mais comum: {placar_mais_comum}")
