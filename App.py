...
# Função para calcular lucro/prejuízo total com base no tipo de equipe e resultados de jogos
...
# Função para calcular odd justa para vitórias apenas
...
# Função para calcular odd justa para vitórias e empates
def calcular_odd_justa_wins_draws(team_df, num_wins, num_draws):
    num_wins_draws = num_wins + num_draws
    odd_justa_wins_draws = team_df.shape[0] / num_wins_draws if num_wins_draws > 0 else 0
...
def mostrar_resultados(team_type, time, odds_column, odds_group):
    ...
    if odds_group[0] == -1 and odds_group[1] == -1:  # Se a opção for "Outros"
        ...
        team_df = team_df[team_df[odds_col] < odds_group[0] and team_df[odds_col] > odds_group[1]]
