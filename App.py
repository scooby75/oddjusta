...
# Função para calcular os resultados importantes
def calcular_resultados_importantes(df, team_type, time, odds_column):
    team_df = df[df[team_type] == time]

    # Filtrar pelo intervalo de odds
    team_df = team_df[team_df[odds_column].between(odds_min, odds_max)]

    # Filtrar pelo intervalo do coeficiente de eficiência
    team_df['Coeficiente_Eficiencia'] = team_df.apply(lambda row: calcular_coeficiente(row, team_type), axis=1)
    team_df = team_df[(team_df['Coeficiente_Eficiencia'] >= coeficiente_min) & (team_df['Coeficiente_Eficiencia'] <= coeficiente_max)]
    ...
    lucro_prejuizo = (num_wins - num_losses)  # Exemplo simples, ajuste conforme necessário
    odd_justa_wins = 1 / (win_percentage / 100) if win_percentage > 0 else 0
    odd_justa_wins_draws = 1 / (num_wins_draws / total_matches) if num_wins_draws > 0 else 0
    coeficiente_eficiencia_medio = team_df['Coeficiente_Eficiencia'].mean()
    media_gols = team_df[['Gols_Home', 'Gols_Away']].mean().mean()
    media_gols_sofridos = team_df[['Gols_Home', 'Gols_Away']].mean().mean()

    return num_wins, num_draws, num_losses, num_wins_draws, total_matches, win_percentage, lucro_prejuizo, odd_justa_wins, odd_justa_wins_draws, coeficiente_eficiencia_medio, media_gols, media_gols_sofridos
...
def main():
    ...
    if team_type == "H2H":
        ...
        odds_min, odds_max = st.sidebar.slider("Intervalo de Odds:", min_value=1.00, max_value=10.00, value=(1.00, 10.00), step=0.01)
        coeficiente_min, coeficiente_max = st.sidebar.slider("Intervalo de Coeficiente de Eficiência:", min_value=-3.00, max_value=3.00, value=(-3.00, 3.00), step=0.01)

        # Calcular resultados importantes
        num_wins, num_draws, num_losses, num_wins_draws, total_matches, win_percentage, lucro_prejuizo, odd_justa_wins, odd_justa_wins_draws, coeficiente_eficiencia_medio, media_gols, media_gols_sofridos = calcular_resultados_importantes(df, team_type, time, odds_column)
        ...
        placar_df = calcular_estatisticas_e_exibir(df[df[team_type] == time], team_type, odds_column)

        # Exibir resultados
        st.subheader(f"Análise do Time {time} - Tipo: {team_type}")
        st.write(f"Total de eventos: {total_matches}")
        st.write(f"Intervalo de Odds Selecionado: ({odds_min}, {odds_max})")
        st.write(f"Intervalo de Coeficiente de Eficiência Selecionado: ({coeficiente_min}, {coeficiente_max})")

        st.write("Distribuição dos Placares:")
